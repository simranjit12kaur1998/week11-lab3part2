...
const addGoal = async (req, res) => {
  try {
    ...
    const newGoal = new Goal({ title, description, targetDate, achieved,user_id });
    await newGoal.save();
...
const updateGoal = async (req, res) => {
  try {
    ...
    const updatedGoal = await Goal.findOneAndUpdate({ _id: id, user_id: user_id }, updatedGoalData, { new: true });
